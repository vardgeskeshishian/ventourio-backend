...
final class CountriesInformationImportService
{
    use InteractWithParsing;
    public function run(): void
    {

        $file = $this->countriesInformationMainPath();
        ...
        try {
            ...
            foreach ($fileToJson as $data) {
                ...
                $country = Country::where('parsing_source', $data['parsing_source'])->first();
                ...
                if($country){
                    ...
                    $this->attachMedia($country, $data['gallery']);
                    $this->attachMedia($country, $data['slider_images'], 'slider');
                }
            }
            DB::commit();
        } catch (\Exception $e) {
